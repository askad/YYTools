...
public class Main {
	...
	/**
	 * 从配置文件读取数据，生成Route对象 read cfg parameter
	 *
	 * @return boolean
	 */
	private static boolean loadCfgFile() {
		try {
			...
			for (; ServiceCount > 0; ServiceCount--) {
				...
				r.LocalPort = Integer.parseInt(pt.getProperty("LocalPort." + ServiceCount).trim());
				r.DestHost = pt.getProperty("DestHost." + ServiceCount).trim();
				r.DestPort = Integer.parseInt(pt.getProperty("DestPort." + ServiceCount).trim());
				r.AllowClient = pt.getProperty("AllowClient." + ServiceCount).trim();
				routeList.add(r);
			}
			is.close();
